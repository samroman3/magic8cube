...
struct Magic8CubeView: View {
    ...
    @State private var blurOpacity: Double = 0.2
    ...
    var body: some View {
        VStack {
            ...
            .onAppear {
                particleManager.createParticles() // Spread out particles initially
                startAnimation()
            }
        }
    }
...
class ParticleManager: ObservableObject {
    ...
    func createParticles() {
        particles.removeAll()
        for _ in 0..<4000 {
            let size = CGFloat.random(in: 0.2...0.9)
            let lifetime = Double.random(in: 1...2)
            let position = CGPoint(x: CGFloat.random(in: 0...200), y: CGFloat.random(in: 0...200))
    ...
    func shimmerParticles() {
        for index in particles.indices {
            particles[index].opacity = Double.random(in: 0.1...1.0)
        }
    }
    
    func reduceParticlesNearTextCenter() {
        particles.removeAll()
        for _ in 0..<4000 {
            let size = CGFloat.random(in: 0.1...0.6)
            let lifetime = Double.random(in: 1...2)
            var position: CGPoint
            
            repeat {
                position = CGPoint(x: CGFloat.random(in: 0...200), y: CGFloat.random(in: 0...200))
            } while distanceToTextCenter(position) < 40 // Ensure fewer particles near the text center
